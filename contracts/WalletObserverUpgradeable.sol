...
import "../implementations/output/OrionsManagerImplementationPointerUpgradeable.sol";
...
contract WalletObserverUpgradeable is
    ...
    OrionsManagerImplementationPointerUpgradeable
{
    ...
    function changeOrionProtocolImplementation(address newImplementation)
        public
        virtual
        override(OrionProtocolImplementationPointerUpgradeable)
        onlyOwner
    {
        super.changeOrionProtocolImplementation(newImplementation);
        excludeFromObserver(orionProtocol.owner(), true);
    }
        ...
        returns (bool)
    {
        ...
        require(
            to != address(orionsManager),
            "WalletObserverUpgradeable: Cannot send directly tokens to orionsManager"
        );
        require(
        ...
        if (!isExcludedFromObserver(to)) {
            ...
            require(
                getMaxTokenPerWallet() >= orionProtocol.balanceOf(to) + amount,
    ...
    function getMaxTokenPerWallet() public view returns (uint256) {
        // 1% - variable
        return (orionProtocol.totalSupply() * maxTokenPerWalletPercent) / 100;
    }
    ...
    function isWalletFull(address account) public view returns (bool) {
        return orionProtocol.balanceOf(account) >= getMaxTokenPerWallet();
    }
